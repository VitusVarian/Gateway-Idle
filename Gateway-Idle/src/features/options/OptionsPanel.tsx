...
export function OptionsPanel({ onSaveNow, onOpenExport, onOpenImport, onOpenReset }: OptionsPanelProps) {
  const totalPlayMs = useGameStore((state) => state.timers.totalPlayMs)

  return (
    <article className="rounded-2xl border border-stone-700/40 bg-stone-900/70 p-4 text-stone-200" aria-label="Options panel">
      ...
      <p className="mt-3 text-sm text-stone-300">
        Total playtime: <span className="font-semibold text-stone-100">{formatDuration(totalPlayMs / 1000)}</span>
      </p>
