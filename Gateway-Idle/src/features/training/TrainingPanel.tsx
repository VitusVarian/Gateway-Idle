  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-sm text-stone-300">
        Training Points:{' '}
        <span className="font-semibold text-stone-100">{formatNumber(trainingPoints)}</span>
      </p>
      ...
      <div className="mt-3 space-y-2">
        {TRAINING_KEYS.map((entry) => (
          <div
            key={entry.id}
            className="flex items-center justify-between rounded-lg border border-stone-700/50 bg-stone-950/50 px-3 py-2"
          >
            <span className="text-sm text-stone-300">{entry.label}</span>
            <span className="text-sm font-semibold text-stone-100">
              Lv {trainingUpgrades[entry.id] ?? 0}
            </span>
          </div>
        ))}
      </div>
